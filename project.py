...
@app.route('/login')
def showLogin():
    """ It authorises the users and show the login page for login"""
...
def gconnect():
    """ it used to login with google and get the access token and one time
    key"""
...
def createUser(login_session):
    """ Here we create user and get user details"""
...
def gdisconnect():
    """ here we disconnect the google account and also clears
     the account passwords and userr ids"""
...
def fbconnect():
    """ it will connect facebook and retrives all the user details from the
    facebook"""
    if request.args.get('state') != login_session['state']:
        response = make_response(json.dumps('Invalid state parameter.'), 401)
        response.headers['Content-Type'] = 'application/json'
...
def fbdisconnect():
    """ it will disconnect the facebook account and clears it
    account details for our site"""
...
def restaurantMenuJSON(restaurant_id):
    """ it shows restaurant menu in Json format"""
...
def menuItemJSON(restaurant_id, menu_id):
    """ it shows menu items of a restaurant in Json"""
...
def deleteMenuItem(restaurant_id, menu_id):
    """ it will delete the menu item of an restaurant only if
they have loged in our site account with third party security"""
    restaurant = session.query(Restaurant).filter_by(id=restaurant_id).one()
